...
func (w *LogFile) ReadLogs(config logger.ReadConfig, watcher *logger.LogWatcher) {
	...
	if config.Tail != 0 {
		...
		tailFile(multireader.MultiReadSeeker(seekers...), watcher, w.createDecoder, config.Tail, config.Since)
	}
	...
	followLogs(currentFile, watcher, notifyRotate, w.createDecoder, config.Since)
}
...
func tailFile(f io.ReadSeeker, watcher *logger.LogWatcher, createDecoder makeDecoderFunc, tail int, since time.Time) {
	var rdr io.Reader
	rdr = f
	if tail > 0 {
...
func followLogs(f *os.File, logWatcher *logger.LogWatcher, notifyRotate chan interface{}, createDecoder makeDecoderFunc, since time.Time) {
